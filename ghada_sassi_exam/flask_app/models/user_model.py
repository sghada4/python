...
class User:
    ...
    #* ***********ALL People who have joined the trip***********
    @classmethod
    def people_joined_trip(cls,data):
        query = """
                SELECT * FROM users
                JOIN joins ON users.id=joins.user_id
                JOIN trips ON trips.id=joins.trip_id
                WHERE joins.trip_id=%(trip_id)s;
                """
        result= connectToMySQL(DATABASE).query_db(query,data)
        print(result)
        if len(result)<1:
